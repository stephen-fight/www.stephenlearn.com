...
Plugin URI: https://wptao.com/blog-optimize.html
...
function blog_optimize_do_page() {
	...
	echo '<div class="updated"><p><strong>请到【<a href="https://wptao.com/blog-optimize.html" target="_blank">点击这里</a>】购买插件，买后卸载本插件，<a href="https://wptao.com/download" target="_blank">重新下载</a>安装后使用。</strong></p></div>';
...
<form class="plugin_options" action="" method="post" id="plugin-options-panel">
  <div class="header">
	<div class="header_left">
	  <h3><a href="https://wptao.com/blog-optimize.html" target="_blank" title="WordPress优化与增强">WordPress</a></h3>
	  <h5>优化与增强</h5>
	</div>
	...
	<div id="tab-base" class="settings"<?php echo (!$authorize['authorize_code']) ? ' style="display:block"' : '';?>>
	  ...
	  <div class="option">
		<?php if ($is_network) { // WPMU
			echo '<span class="text-tips">该插件已经被 《<a href="' . get_site_option('siteurl') . '" target="_blank">' . get_site_option('site_name') . '</a>》授权允许在整个网络使用，插件提供者: <a href="https://wptao.com/blog-optimize.html" target="_blank">水脉烟香</a>。</span>';
		} else {
			echo '<h4>填写插件授权码</h4><input type="text" class="inputs" name="authorize_code" size="50" value="' . $authorize['authorize_code'] . '" /> ' . $code_yes;
			if (is_multisite()) echo '<span class="text-tips">您正在使用WPMU，您可以在“管理网络”的插件页面“整个网络启用“博客优化”，然后在 管理网络 -> 设置 -> <a href="' . admin_url('network/settings.php?page=blog-optimize') . '">博客优化</a> 填写插件“根域名”授权码。<a href="https://wptao.com/blog-optimize.html" target="_blank">如何获得根域名授权码</a></span>';
		}?>
	  </div>
    </div>
